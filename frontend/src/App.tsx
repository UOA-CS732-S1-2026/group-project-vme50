...
function App() {
  ...
  async function refreshSessions() {
    ...
    try {
      const response = await fetchJson<ApiEnvelope<MealSession[]>>(`${API_BASE_URL}/api/meals`)
      setSessions(response.data.map(normalizeMealSession))
    } catch (error) {
  ...
  async function syncSessionFromServer(sessionId: string) {
    try {
      const response = await fetchJson<ApiEnvelope<MealSession>>(`${API_BASE_URL}/api/meals/${sessionId}`)
      const normalizedSession = normalizeMealSession(response.data)
  ...
  async function handleAuthSubmit(event: FormEvent<HTMLFormElement>, mode: AuthMode) {
    ...
    try {
      const response = await fetchJson<ApiEnvelope<{ token: string; user: SessionUser }>>(
      ...
      const nextProfile = normalizeUserProfile(response.data.user)
  ...
  async function handleCreateSession(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const fallbackCoordinates = getFallbackCoordinates(sessionForm.location.trim())
      const coordinates = sessionLocationCoordinates ?? fallbackCoordinates
      ...
      await fetchJson<ApiEnvelope<MealSession>>(`${API_BASE_URL}/api/meals/create`, {
        method: 'POST',
        body: payload,
        token,
      })
  ...
  async function handleSessionAction(sessionId: string, action: SessionAction) {
    ...
    try {
      const response = await fetchJson<ApiEnvelope<MealSession | null> & { deleted?: boolean; sessionId?: string }>(
      ...
      if (response.deleted || !response.data) {
        const removedSessionId = response.sessionId || sessionId
        setSessions((current) => current.filter((session) => getSessionId(session) !== removedSessionId))
        setGlobalNotice(response.message || 'Left session.')
        return null
      }
      ...
      setGlobalNotice(response.message || (action === 'join' ? 'Joined session.' : 'Left session.'))
  ...
  async function handleCloseSession(sessionId: string) {
    ...
    try {
      const response = await fetchJson<ApiEnvelope<MealSession>>(
        `${API_BASE_URL}/api/meals/${sessionId}/close`,
        {
          method: 'POST',
          token,
        },
      ...
      setGlobalNotice(response.message || 'Session closed.')
      return normalizedSession
    } catch (error) {
      const message = getErrorMessage(error)
      setSessionError(message)
  ...
  async function handleProfileSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetchJson<ApiEnvelope<UserProfile>>(`${API_BASE_URL}/api/auth/profile`, {
        method: 'PATCH',
        body: profileForm,
        token,
      })
...
function SessionDetailsPage({
  ...
  useEffect(() => {
    ...
    void fetchJson<ApiEnvelope<MealSession>>(`${API_BASE_URL}/api/meals/${sessionId}`)
      .then((response) => {
        setSession(normalizeMealSession(response.data))
      })
      .catch((error) => {
...
function CreateSessionPage({
  ...
  const slotCount = Math.max(1, Number(sessionForm.slots) || 4)
  ...
  const decreaseSlots = () => {
    ...
    onSessionFormChange('slots', String(Math.max(1, slotCount - 1)))
  }
  const increaseSlots = () => {
    if (slotCount >= 20) {
      return
    ...
    onSessionFormChange('slots', String(Math.min(20, slotCount + 1)))
  }
...
function ProfilePage({
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchJson<ApiEnvelope<MealSession[]>>(`${API_BASE_URL}/api/meals/mine/hosting`, { token }),
      fetchJson<ApiEnvelope<MealSession[]>>(`${API_BASE_URL}/api/meals/mine/joined`, { token }),
    ])
      .then(([hosting, joined]) => {
        setHostingSessions(hosting.data.map(normalizeMealSession))
        setJoinedSessions(joined.data.map(normalizeMealSession))
      })
      .catch((error) => {
