...
export const getAllMeals = async (req, res) => {
    try {
        await MealSession.updateMany({ isActive: true, time: { $lte: new Date() } }, { isActive: false });
...
export const getMyHostedMeals = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
...
export const getMyJoinedMeals = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
...
export const joinMealSession = async (req, res) => {
    try {
        const userId = req.user?.userId;
...
export const leaveMealSession = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const sessionId = req.params.id;
        if (!userId) {
...
export const closeMealSession = async (req, res) => {
    try {
